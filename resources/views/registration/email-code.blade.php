...
<main class="registration-page">
    <div class="registration-shell">
        <section class="registration-card verified-card" aria-labelledby="emailCodeTitle">
            ...
            <form method="POST" action="{{ route('registration.email-code.verify') }}" class="mx-auto text-start" style="max-width: 430px;" data-verification-code>
                @csrf
                <label class="form-label" for="code">Código de verificación</label>
                <input class="form-control @error('code') is-invalid @enderror" id="code" name="code" type="tel" inputmode="numeric" pattern="[0-9]{6}" maxlength="6" autocomplete="one-time-code" autofocus required>
                <div class="form-text">Ingresa los seis dígitos recibidos.</div>
                @error('code')<div class="invalid-feedback">{{ $message }}</div>@enderror
                <div class="registration-form-actions mt-4"><button class="btn btn-primary" type="submit" disabled>Verificar y continuar</button><a class="btn btn-primary" href="{{ route('registration.identity') }}">Volver al inicio</a></div>
            </form>
