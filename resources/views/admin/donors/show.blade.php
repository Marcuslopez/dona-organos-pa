...
<div class="admin-app donor-record-page">
    ...
    <main class="admin-main"><div class="container-fluid admin-container">
        ...
        <div class="detail-stack">
            ...
            <section class="admin-panel detail-card"><h2>Consentimiento</h2><dl class="detail-list detail-list-horizontal consent-detail"><div><dt>Estado</dt><dd>{{ $consent?->accepted ? ($consent?->revoked_at ? 'Revocado' : 'Aceptado') : '—' }}</dd></div><div><dt>Secuencia</dt><dd>{{ $consent?->consent_sequence ?: '—' }}</dd></div><div><dt>Versión</dt><dd>{{ $consent?->version ?: '—' }}</dd></div><div><dt>Fecha de aceptación</dt><dd>{{ $consent?->accepted_at ? \Carbon\Carbon::parse($consent->accepted_at)->timezone('America/Panama')->format('d/m/Y H:i') : '—' }}</dd></div></dl><details class="consent-audit-details"><summary>Ver información técnica</summary><dl class="detail-list detail-list-horizontal consent-detail"><div><dt>ID de solicitud</dt><dd>{{ data_get($consent, 'request_id') ?: 'No disponible para este consentimiento histórico' }}</dd></div><div><dt>Dirección IP</dt><dd>{{ data_get($consent, 'ip_address') ?: 'No disponible' }}</dd></div><div><dt>Navegador informado</dt><dd>{{ data_get($consent, 'user_agent') ?: 'No disponible' }}</dd></div></dl></details></section>
